...
class Eliza(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if(message.content == '!konsultasi'):
            ...
            await message.channel.send(self.initial())
            return
        ...
        if(self.konsultasi): 
            ...
            if text.lower() in self.quits:
                await message.channel.send(self.final())
            ...
            for key in keys:
                self.output = self._match_key(words, key)
                if self.output:
                    log.debug('Output from key: %s', self.output)
                    break
            if not self.output:
                if self.memory:
                    index = random.randrange(len(self.memory))
                    self.output = self.memory.pop(index)
                    log.debug('Output from memory: %s', self.output)
                else:
                    self.output = self._next_reasmb(self.keys['xnone'].decomps[0])
                    log.debug('Output from xnone: %s', self.output)
            
            self.output = " ".join(self.output)
            await message.channel.send(self.output)
